    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
        <motion.div
          ...
          className="relative w-full max-w-lg overflow-hidden rounded-2xl border-2 border-[#3FA97A] bg-gradient-to-b from-[#2B1B17] via-[#1F1410] to-[#120B09] p-6 shadow-2xl text-[#A8E6C8]"
          style={{ fontFamily: "'Cinzel', serif" }}
        >
          {/* Header Crown Banner */}
          <div className="flex items-center justify-between border-b border-[#3FA97A]/30 pb-4 mb-4">
            <div className="flex items-center gap-3">
              <div className="p-2.5 rounded-full bg-[#3FA97A]/20 border border-[#3FA97A]/50 text-[#4FD1A0]">
            ...
            <button
              onClick={onClose}
              disabled={submitting}
              className="p-1 text-[#A8E6C8]/60 hover:text-[#4FD1A0] hover:bg-[#3FA97A]/10 rounded-lg transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          {resultStatus === 'accepted' ? (
            ...
            >
              ...
              <p className="text-sm text-[#A8E6C8]/80 max-w-sm mx-auto">
                The Central Hub has validated your identity. Letter safely sealed in your mailbox and +20 XP awarded to Postman {data.mailmanName || 'Royal Mailman'}.
              </p>
            </motion.div>
            ...
            >
              ...
              <p className="text-sm text-[#A8E6C8]/80 max-w-sm mx-auto">
                Central Hub has flagged this misdelivery. Postman {data.mailmanName || 'Royal Mailman'} has received a formal penalty (-15 XP).
              </p>
            </motion.div>
          ) : (
            <>
              {/* Central Hub Verification Card */}
              <div className="bg-[#120B09]/80 border border-[#3FA97A]/30 rounded-xl p-4 mb-5 space-y-3">
                <div className="flex items-center justify-between text-xs border-b border-[#3FA97A]/20 pb-2">
                  <span className="text-[#3FA97A] font-semibold flex items-center gap-1.5">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm pt-1">
                  <div>
                    <span className="text-xs text-[#A8E6C8]/60 block">Sender of Record:</span>
                    <span className="font-semibold text-amber-100 flex items-center gap-1.5">
                      <FileText className="w-3.5 h-3.5 text-[#3FA97A]" /> {data.senderName || 'Noble Scribe'}
                    </span>
                  </div>
                  <div>
                    <span className="text-xs text-[#A8E6C8]/60 block">Postman Presenting:</span>
                    <span className="font-semibold text-amber-100 flex items-center gap-1.5">
                      <UserCheck className="w-3.5 h-3.5 text-[#3FA97A]" /> {data.mailmanName || 'Royal Mailman'}
                    </span>
                  </div>
                </div>

                <div className="text-xs bg-[#2B1B17]/60 p-2.5 rounded border border-[#3FA97A]/20 text-[#A8E6C8]/90 italic">
              ...
              {isDeclining ? (
                <div className="space-y-3 mb-4">
                  <label className="text-xs text-red-300 font-semibold block">
                    Reason for Declining Custody (Postman will incur -15 XP penalty):
                  </label>
                  <textarea
                    value={declineReason}
                    onChange={(e) => setDeclineReason(e.target.value)}
                    placeholder="E.g., Wrong recipient name, physical envelope damaged, postman did not present letter..."
                    rows={3}
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <button
                    ...
                    disabled={submitting}
                    className="flex-1 py-3 px-4 bg-gradient-to-r from-[#3FA97A] via-[#A8E6C8] to-[#1D6B4C] text-[#2B1B17] hover:brightness-110 font-bold rounded-xl shadow-lg border border-[#A8E6C8] flex items-center justify-center gap-2 transition-all"
                  >
