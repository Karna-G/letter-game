...
export default function AdminDashboard() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // FIX: Send EXACTLY what the Admin typed, with no extra added text!
      const fullMessage = messageContent;
      
      const response = await fetch(`http://localhost:5000/api/users/reports/${messageModal.reportId}/verdict`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      setMessageContent("");
      fetchAdminData(); // Refreshes the table
    } catch (err: any) {
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10 p-8 bg-[#FDF5E6] border-4 border-[#8B5A2B] rounded-lg shadow-2xl mb-20 relative">
      ...
      <p className="text-center text-[#8B5A2B] mb-8 uppercase tracking-widest text-sm font-semibold">Campus Postal Network Overview</p>
      
      {/* --- TAB NAVIGATION --- */}
      <div className="flex justify-center gap-4 mb-10 border-b-2 border-[#D2B48C] pb-6">
        <button 
            ...
            <table className="w-full text-left border-collapse relative">
              <thead className="sticky top-0 bg-[#FAF0E6] shadow-sm z-10 border-b-2 border-[#8B5A2B]">
                <tr>
                  ...
                  <th className="p-3 text-[#5C3A21] font-bold uppercase text-sm">Accused Target</th>
                  <th className="p-3 text-[#5C3A21] font-bold uppercase text-sm">Transgression</th>
                  <th className="p-3 text-[#5C3A21] font-bold uppercase text-sm">Status</th>
                ...
                {reports.length === 0 ? (
                  ...
                  reports.map((report) => (
                    <tr key={report._id} className={`border-b border-[#D2B48C] transition ${report.status === 'pending' ? 'bg-red-50 hover:bg-red-100' : 'hover:bg-[#FDF5E6] opacity-60'}`}>
                      ...
                      <td className="p-3 text-red-700 font-bold">{getIdentity(report.reportedUser)}</td>
                      <td className="p-3 text-sm italic max-w-xs">{report.reason}</td>
